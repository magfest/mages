...
class Root:
    def index(self, session, message='', **params):
        app = session.mages_application(params, checkgroups={'tech_needs'}, restricted=True)
        panelist = session.mages_applicant(params, restricted=True)
        ...
        for i in range(int(params.get('other_panelists', 0))):
            ...
            other_panelists.append(MagesApplicant(application=app, **applicant))
